  ...
  return (
    <div className="p-6 bg-green-50 shadow-lg rounded-lg border-2 border-gray-400">
      <h2 className="text-2xl font-bold mb-4 flex items-center">
        <FaChartBar className="mr-2" /> Exam Results
      </h2>
      <table className="w-full table-auto bg-white shadow-md">
        ...
        <tbody>
          {results.map((result, index) => (
            <tr key={index}>
              <td className="border px-4 py-2 text-gray-700">{result.subject}</td>
              <td className="border px-4 py-2 text-gray-700">{result.score}</td>
            </tr>
          ))}
        </tbody>
      </table>
